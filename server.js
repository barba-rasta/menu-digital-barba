...
// Subida de imágenes en memoria temporal / estática
...
// Conexión a MySQL (Aiven Managed Database - SSL requerido)
let db;
async function initDB() {
  const isAiven = (process.env.DB_HOST || '').includes('aivencloud.com');

  db = await mysql.createPool({
    host: process.env.DB_HOST || 'mysql-3b3881a4-barbanesta-ca4d.aivencloud.com',
    port: Number(process.env.DB_PORT) || 28935,
    user: process.env.DB_USER || 'avnadmin',
    password: process.env.DB_PASSWORD || 'MenuPass123',
    database: process.env.DB_NAME || 'defaultdb',
    ssl: isAiven || process.env.DB_HOST ? { rejectUnauthorized: false } : false,
  ...
  console.log('Base de datos conectada correctamente en Aiven');
}
...
app.post('/api/register', async (req, res) => {
  try {
    if (!db) {
      return res.status(503).json({ error: 'Base de datos no conectada. Configura DB_HOST, DB_USER, DB_PASSWORD y DB_NAME.' });
    }

    const { name, slug, email, password, phone, description } = req.body;

    if (!name || !slug || !email || !password) {
      return res.status(400).json({ error: 'Faltan campos obligatorios (nombre, slug, email, contraseña)' });
    }
    ...
    if (err.code === 'ECONNREFUSED' || err.code === 'ENOTFOUND') {
      return res.status(503).json({ error: 'No se pudo conectar a MySQL. Verifica que el servicio esté activo.' });
    }
...
app.post('/api/login', async (req, res) => {
  try {
    if (!db) {
      return res.status(503).json({ error: 'Base de datos no conectada. Configura las variables de entorno de MySQL.' });
    }
...
app.put('/api/business/profile', authMiddleware, upload.fields([{ name: 'banner' }, { name: 'logo' }]), async (req, res) => {
  try {
    ...
    let bannerImage = req.files && req.files['banner'] ? `/uploads/${req.files['banner'][0].filename}` : null;
    let logoImage = req.files && req.files['logo'] ? `/uploads/${req.files['logo'][0].filename}` : null;
