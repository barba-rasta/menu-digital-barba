...
// Conexión a MySQL (Akamai Managed Database - SSL requerido)
let db;
async function initDB() {
  db = await mysql.createPool({
    host: process.env.DB_HOST || '127.0.0.1',
    port: Number(process.env.DB_PORT) || 3306,
    user: process.env.DB_USER || 'menu',
  ...
  console.log('Base de datos conectada correctamente en Wasmer');
}
...
app.get('/api/admin/businesses', adminMiddleware, async (req, res) => {
  try {
    ...
    // Contar productos y comentarios por negocio
    for (const b of rows) {
